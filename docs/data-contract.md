...
| — | 2026-09-07 | Not a re-pin — **Pinned Contract Version stays 1.10.0.** `EN.14.I` (engine-rs-side) gives engine-rs its own `events` table (`crates/engine-store/migrations/0004_create_events.sql`), so an engine database can be stood up from engine-rs's own migrations alone — no Synapse checkout, no alembic anywhere in the path. Per brain D84 + Amendment 1, `events` is the workflow-dispatch table for BOTH runtimes: engine-rs does NOT take over Synapse's table, and Synapse keeps its own `events` for its four Brain workflows (`DOCUMENT_INGEST`, `DOCUMENT_QA`, `MEMORY_INGEST`, `MEMORY_CONSOLIDATION` — `core/synapse/app/api/schema_registry.py:9-14`, `app/worker/tasks.py::process_incoming_event`); no file under `../synapse/` is touched by this change. Column types were diffed against the live `orchestration_dev` schema rather than authored from this document (`planning/EN.14.I/live-schema-diff.md`): `data`/`task_context` are `json`, NOT `jsonb`, matching `postgres.rs`'s `get_task_context`/`list_orphan_candidates`; `data`, `task_context`, `created_at` and `updated_at` are all NULLABLE, matching the live table's measured nullability rather than `EventsRow`'s non-`Option` Rust fields (`EventsRow` itself is unchanged — this row is a table-provenance and nullability correction, not a shape change). **No re-pin**: no `engine_contract` `events`/`task_context`/`NodeRun`/`Usage` Rust type changed shape, and `upsert_event`'s `INSERT ... ON CONFLICT (id) DO UPDATE` behaviour is unchanged — only the table's provenance moves from "the orchestrator's existing schema" to "engine-rs's own migration". See [engine-store/tests/migrations_apply_cleanly.rs](../crates/engine-store/tests/migrations_apply_cleanly.rs) for the from-empty standup test asserting all three tables (`events`, `journal`, `node_invocations`) together, and its positive control proving that assertion can actually fail. |
| — | 2026-09-12 | Not a re-pin — **Pinned Contract Version stays 1.10.0.** `EN.16.B` adds an additive `cost_known: bool` to `ClaudeSession` (`crates/engine-core/src/sessions.rs`), each `metadata.claude_sessions[]` entry's ledger-entry type (§ Run-level `metadata` annotations above), with `#[serde(default = "default_cost_known")]` returning `true` — every pre-existing ledger entry was a real, priced Claude CLI cost, so it deserializes as known. It exists because this block gives `ImplementTaskNode` a second transport, `PiTransport` (a local Ollama-backed coding agent dispatched via `AgentBackend::Pi`, see [docs/workflows/README.md](workflows/README.md)), whose subprocess can report tokens with no dollar figure at all — `cost_known: false` records that gap explicitly rather than letting an unpriced invocation collapse into an indistinguishable `cost_usd: 0.0`. `sessions::ledger_totals` now also counts unknown-cost invocations, and `RunTelemetry` surfaces that count so a run mixing Pi and Claude CLI invocations reports a visibly incomplete dollar total rather than a bare number understating it. Mirrors the same free-form-metadata reasoning as the 2026-08-31 rows above: **no re-pin**, because the field lives in the existing `TaskContext::metadata` (D6) and the artifact is engine-rs's own `sdlc-*state.json`, not one of the versioned `events`/`task_context`/`NodeRun`/`Usage` shapes this pin tracks. Additive and backward-compatible — a ledger entry written before this block has no such key and every consumer must read it as `cost_known: true` (a real, already-priced cost), never as unknown. |
