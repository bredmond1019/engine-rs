...
| Flag | Default | Notes |
|---|---|---|
| `--models` | required | Comma-separated names, a JSON array (`'["qwen3:14b","gpt-oss:20b"]'`), or `all` (every pulled model, smallest first, base variants only; `-ctxN` variants skipped; `RETIRED_MODELS` excluded -- see `--include-retired` and `--require-capability` for capability filtering) |
| `--include-retired` | off | Include `RETIRED_MODELS` in an `--models all` sweep. Naming a retired model explicitly (`--models llama3.2:3b`) always runs it regardless of this flag |
| `--tiers` | `easy,edit,medium,hard,rust` | |
| `--agent-backends` | `aider,pi` | Comma-separated: `aider`, `pi`, or `aider,pi`. Pass a single backend (e.g. `--agent-backends pi`) to run only that one |
...
| 21 | In `overnight-sandbox-2026-09-14`, every model's `aider` job ran first on a freshly-cleaned worktree/block slot and (almost) always passed; its `pi` job ran immediately after, reusing the same slot, and took nearly every infra-shaped loss (`no_change` with 0 attempts/0 tokens, worktree files missing, generic `node '<unknown>' did not succeed` bail) — contaminating the aider-vs-pi comparison, since `pi` was never tested on equal footing | `run_one_job_orchestration`'s end-of-job block reopen (`mev set-block-status ... open --write`) was fire-and-forget — a failed reopen left the block wrong for the *next* job, which then bailed with a generic error attributing nothing to the job that actually caused it. The two backends also loaded different Ollama model variants back-to-back with no eviction between them, a plausible resource-contention confound | Bench: the end-of-job reopen is now checked and logged loudly against the job that caused it (not silently swallowed); `unload_ollama_model` evicts the just-used model (`keep_alive: 0`) at the end of **every** job in both dispatch modes, regardless of what runs next. Root cause not fully proven — re-run `pi`-only (`--agent-backends pi`) to get a clean read once this lands |
| 22 | `qwen2.5:3b` and `llama3.2:3b` (the two smallest models in the sweep) accounted for most of the non-infra-adjacent losses in `overnight-sandbox-2026-09-14`, on both backends, including on the trivial tiers | Model capacity, not a harness bug — confirmed on jobs unaffected by #21 (e.g. `qwen2.5:3b` scored `infra` on both backends across the board; `llama3.2:3b` passed only `easy`) | Bench: both retired from `--models all` via `RETIRED_MODELS` (documented reasons inline); `--include-retired` re-adds them, and naming either explicitly always runs it |
| 23 | Update to #21: a `pi`-only rerun (`pi-rerun-2026-09-15`, no aider ever ran first) still scored `qwen2.5-coder:14b`/`deepseek-r1:14b` 0/3 with the same `no_change` signature — so aider-before-pi ordering was **not** the (sole) cause for these two models. `gpt-oss:20b`/`qwen3:8b` passed 3/3 clean in the same rerun | Genuinely model-quality: the per-job record showed 4 real attempts and `changed_paths: []` — the model made zero file changes across every attempt. (A separate investigation into why the engine reported this as an opaque `node '<unknown>' did not succeed` rather than the real "max attempts exhausted" reason is #24) | Not a bench bug — #21's reopen/eviction fix still stands as real hardening, it just wasn't the explanation for these two models specifically |
| 24 | The ORCHESTRATION chain's own failure attribution reported the unhelpful `node '<unknown>' did not succeed` for a run that legitimately exhausted its attempts — `WrapUpNode`→`CloseBlockNode`→`PullRequestNode`→`EmitStateNode` all dispatch and succeed as *nodes* on this path (`WrapUpNode` just stamps `metadata.failure`, it doesn't fail), so the attribution logic's search for a literally-failed node found nothing and fell back to the placeholder | `execute_step`'s `ChildFailed` attribution only checked `ctx.node_runs` for `NodeRunStatus::Failed`, never `metadata.failure` | **Fixed** (engine, not bench): `crates/engine-core/src/workflows/orchestration/execute.rs` now falls back to `metadata.failure.error` before the placeholder. Regression test added |
| 25 | Orchestration-mode (`--dispatch orchestration`) evidence capture never copied a job's `sdlc/` state directory into `artifacts/<job>/`, even for jobs whose own JSON record (`total_attempts`, `tasks`, etc.) was correct | `capture_evidence()` hardcoded the module-level `REPO_DIR` (this script's own live `core/engine-rs` checkout) to locate `planning/<work_id>/sdlc/`, but an orchestration-mode job runs against the **sandbox's** own engine-rs checkout — a different filesystem location entirely. `harvest_state()` was already sandbox-aware (it takes an explicit state-file path), so only the artifact-copy step was affected | Bench: `capture_evidence()` takes an explicit `repo_dir` parameter (`None` defaults to `REPO_DIR`, resolved at call time so it stays test-patchable); `run_one_job_orchestration` passes the sandbox's `engine_rs_dir` |
