...
| [research-agent-workflow.md](research-agent-workflow.md) | The `RESEARCH_AGENT` graph — dual-mode router (company brief vs. prospecting) terminating in a `MaterializeDocNode` write followed by a `MergeContactsNode` contact merge, event schema (incl. the `EN.4.F` `locale` field + language directive), tunable `ResearchAgentPolicy` + named profiles including the `contact_enrichment` acquisition-depth knob, the contact-extraction anti-fabrication contract and two-step write, `url`/`links` source lifting, triggering, and reading `research-agent-state.json` outputs |
