...
| [content-pipeline-workflow.md](content-pipeline-workflow.md) | The `CONTENT_PIPELINE` graph — thirteen-node envelope-based, channel-agnostic content workflow (fetch/normalize -> summarize -> bounded self-critic loop -> optional translate -> digest render -> persist), the `IngressEnvelope` contract, event schema, tunable `ContentPipelinePolicy` (with Local-tier rewires) + named profiles, the engine↔brain `PersistToBrainNode` boundary, triggering, and reading outputs |
