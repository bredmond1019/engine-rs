...
| [research-agent-workflow.md](research-agent-workflow.md) | The `RESEARCH_AGENT` graph — dual-mode router (company brief vs. prospecting) terminating in a `MaterializeDocNode` write, event schema, tunable `ResearchAgentPolicy` + named profiles, triggering, and reading `research-agent-state.json` outputs |
