...
| `GenerateTasksNode` | **Model** (Opus, tunable via policy) | Two decomposition paths (`EN.19.C`), chosen by an EXACT filename match only (never fuzzy) on `planning/blocks/{spec_slug}.json`: when that file exists, gathers the block record's `what`/`files`/`acceptance_criteria`/`out_of_scope`/`interfaces` and decomposes from them, resolving its model through the SEPARATE `model_tiers.generate_from_block` knob (a direct field read, not routed through `apply_policy`'s `Stage::Generate`); otherwise falls back to the original planning-fallback path — gathering `planning/<slug>/*.md` context — byte-for-byte unchanged, and resolving its model through `model_tiers.generate` via `apply_policy(..., Stage::Generate)` as before. Either path writes `tasks.json` + `tasks.md`. Sets `config.json_schema` and prefers the model's structured output (`ctx.nodes["GenerateTasksNode"]["structured"]`) over fence-stripped text parsing, falling back to `strip_json_fence` + `serde_json::from_str` when structured output is absent. Reads `timeouts.generate` on both paths; Opus is the built-in default for both `model_tiers.generate` and `model_tiers.generate_from_block`, chosen to be byte-identical to the removed hardcode — tuning one knob never silently changes the other's resolved model. |
...
| `ConsolidatedReviewNode` | **Model** (Sonnet, tunable via policy) | Reviews the task's **working-tree diff against `HEAD`** (`git add -N -A`, then `git diff HEAD` — see [Commit topology and what the reviewer sees](#commit-topology-and-what-the-reviewer-sees)) against acceptance criteria; parses `{verdict, summary, issues, localized}`, preferring the model's structured output (`ctx.nodes["ConsolidatedReviewNode"]["structured"]`) over fence-stripped text parsing and falling back to `strip_json_fence` + `serde_json::from_str` when structured output is absent. Reads policy for model tier/prompt-cache/verbosity; can be rewired to the `local` tier. |
...
### Stage prompts: what the model is actually told

Four of the model nodes prepend a **run-invariant preamble** to their per-call prompt. These are
ported from the JS engines (`base-template/.claude/workflows/`), which had 37-88 line prompts where
the Rust engine had 4-6 — a thinness that produced real misclassifications, not just terser text.

| Node | Constant | What the preamble adds |
|---|---|---|
| `ImplementTaskNode` | `PATH_DISCIPLINE_PREAMBLE` + `IMPLEMENT_STANDARDS_PREAMBLE` (`task_loop.rs`) | Which tree to write in; CLAUDE.md is the authority; scope to this task only; the D8 completeness self-check (no `todo!()`/stub on a path an acceptance criterion requires). |
| `TriageTaskNode` | `TRIAGE_STABLE_PROMPT` (`task_loop.rs`) | The five immediate-bail reasons; "bailing is cheap, a wasted retry loop is not"; the evidence clause (re-run the failing check against base state before claiming a failure pre-dates the task); the caution that harness-created workspace state is a *candidate cause*, not a fixed backdrop. |
| `ConsolidatedReviewNode`, `EndReviewNode` | `REVIEW_STABLE_PROMPT` (`task_loop.rs`, shared) | Mark each criterion MET/PARTIAL/NOT_MET against source with cited evidence; a run-state summary is an index, not evidence; a criterion is met only as **written**, never as a weaker paraphrase that happens to hold. |
| `PatchDocsNode` | `DOCS_STABLE_PROMPT` (`docs.rs`) | Surgical patching only (never rewrite a whole file, source wins over the doc); BOOTSTRAP MODE for a repo with no `docs/` at all; the `write-repo-doc` gap list; flag a top-level doc rather than editing it. |

**What was deliberately NOT ported, and why it matters.** The JS prompts also tell the model to
re-run the gating suite, commit its own work, and assert that its commit contains the task's work.
None of that came across, because engine-rs already enforces all three **in Rust** — `run_checks`,
`commit_all`, and `verify_claimed_writes`. Moving an enforced invariant back into prose a model can
skip would be a downgrade wearing the costume of parity.

The most visible consequence is in the review verdict definitions, which rule on the **criteria
alone** — every gating clause was stripped. That is stricter than the JS engine, not looser: the
model cannot pass a run whose gates failed, because it is not the thing consulting the gates.

**Cache discipline.** Each constant is a literal with no `format!` interpolation, so it is a
byte-stable prefix on every call (`CLAUDE.md` standing rule 6). Per-run values — the task title, the
failure summary, the diff — always go in the per-call body. A unit test per stage asserts the
constant contains no `{`, which turns a future cache regression into a test failure rather than
...
- **Docs patch** — `PatchDocsNode`'s output (`{summary, files_patched, created, flagged}`) records which docs it
