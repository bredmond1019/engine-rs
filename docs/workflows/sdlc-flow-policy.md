...
| `model_tiers.implement_final_attempt` | `sonnet` \| `haiku` \| `opus` \| `local`, or omitted (`null`) | **`EN.17.F`.** Escalates `ImplementTaskNode` to this tier on a task's LAST attempt only (`attempt_count + 1 == max_attempts`); every earlier attempt still uses `model_tiers.implement`. Built-in default `None` (every attempt stays on `implement` — behavior-stable). Mirrors the JS `/sdlc-flow` engine's `ESCALATION_MODEL` behavior (`.claude/workflows/sdlc-flow.js`), which always escalates a failing final attempt to Opus; HQ's `orchestration.policy.child_sdlc_flow_policy` sets this to `opus` for a Rust chain's SDLC_FLOW children, for JS parity. The tier actually used is stamped into `ctx.nodes["ImplementTaskNode"]["model_tier"]` (CLAUDE.md standing rule 6). |
| `max_turns.{implement,triage,review,generate,docs}` | `u32`, or omitted (`null`) | **`EN.17.F`.** Per-stage in-turn tool-call ceiling, passed straight through to `claude_code_rs::Config::max_turns` (`CC.ticket.config-max-turns-passthrough`). Built-in default all-`None` — `claude-code-rs`'s own unbounded default, unchanged. Exists because an unbounded turn budget is a real cost incident: one bella run made 156 tool calls in a single turn before this knob existed. Mirrors `timeouts.*` in shape (same five stages, same `apply_policy_config` call site). |
| `generate_context_max_bytes` | `usize`, or omitted (`null`) | **`EN.17.F`.** Caps each spec `.md` file `gather_context` (`GenerateTasksNode`'s planning-fallback path — used only when a spec has no `tasks.json` yet) inlines into its prompt. A file over the cap is truncated at a UTF-8 boundary with a marker naming the file and the bytes dropped (`...[truncated: N bytes dropped from <file>]`); a file under the cap is embedded verbatim, unmarked. Built-in default `None` — output is byte-identical to before this knob existed. |
...
| `agent_backend` | `ClaudeCli` (default) \| `Pi` \| `Aider` | **`EN.16.D`, extended `EN.16.C`.** Which coding agent `ImplementTaskNode`'s dispatch uses on SDLC_FLOW — `ClaudeCli` (default, billed), `Pi`, or `Aider` (`EN.16.C`; both of the latter run against a local Ollama model, cost reported as unknown rather than a false `$0.00`). Resolves through the same four layers as every other knob (per-run `policy` override > named `profile` bundle > `sdlc.policy` harness default > built-in default) and is honored on both of `registry_for_policy_with_cancellation`'s registration paths, including `token: None`. Originally carried only on `SdlcTaskPolicy` (`EN.16.B`) — see [`agent_backend`](README.md#agent_backend--which-coding-agent-sdlc_tasks-implement-stage-drives) for the full mechanism and the safety boundary, which applies unchanged here. |
...
- `unknown_cost_invocations` (**`EN.16.B`**) — count of ledger invocations recorded with
  `cost_known: false` (a non-`claude_cli` `agent_backend`, e.g. `PiTransport` or `AiderTransport`, that reports no
