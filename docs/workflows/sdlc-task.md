...
| `agent_backend` | `ClaudeCli` | **`EN.16.B`.** Which coding agent `ImplementTaskNode`'s dispatch uses — `ClaudeCli` (default, billed) or `Pi` (`pi_agent_rust` against a local Ollama model, cost reported as unknown rather than a false `$0.00`). See [`agent_backend`](README.md#agent_backend--which-coding-agent-sdlc_tasks-implement-stage-drives) for the full mechanism and the safety boundary. Not carried on `SDLC_FLOW`'s own policy surface yet — that dispatch wiring is a later block (`EN.16.D`). |
