...
| `agent_backend` | `ClaudeCli` | **`EN.16.B`, extended `EN.16.C`.** Which coding agent `ImplementTaskNode`'s dispatch uses — `ClaudeCli` (default, billed), `Pi` (`pi_agent_rust` against a local Ollama model), or `Aider` (`aider`, also against a local Ollama model, `EN.16.C`) — the latter two report cost as unknown rather than a false `$0.00`. See [`agent_backend`](README.md#agent_backend--which-coding-agent-sdlc_tasks-implement-stage-drives) for the full mechanism and the safety boundary. Carried on `SDLC_FLOW`'s own policy surface too (`EN.16.D`). |
