...
## The lane-log line and `planning/state.json` no longer disagree

Before `EN.ticket.orchestration-close-block-node-not-wired`, a chain step's completion was recorded
ONLY in `lane-log.jsonl` (the "The lane-log contract" section above) — `planning/state.json`'s
authored block `status` was never touched by `ORCHESTRATION`, even though `CloseBlockNode` already
did exactly this for standalone `SDLC_TASK`/`SDLC_FLOW` runs. The gap: a dependency-gated chain's
SECOND block reads its dependency edge through `corpus_gates.rs`, which reads `state.json`, not the
lane log — so a real, merged first block left its own `state.json` entry `"open"`, and the second
block's `is_edge_met` check reported the edge unmet forever, with no error to explain it (the two
sources of truth simply disagreed).

`integrate_chain`'s per-step completion path now calls a `close_block` seam
(`integrate::CloseBlockFn`) with `(repo, block_id)` exactly once per successfully-integrated `block`
step — after that step's `lane-log.jsonl` `closed` line, before the next step's dependency gate.
Threaded from `OrchestrationRunNode::with_close_block(...)` (mirroring
`with_resolve_depends_on`/`with_is_edge_met`/`with_is_block_open`/`with_hold_source`), production
registration (`engine-serve::workflows::register_orchestration_with_registry`) wires a real
closure — `build_close_block_seam` — that calls
`sdlc_flow::close_block::close_block_direct(brain_root, repo, block_id)`: the SAME guarded write
`CloseBlockNode::process` performs for `SDLC_TASK`/`SDLC_FLOW` (advisory `.mev-emit.lock`, the D71
operator gate, validate-then-rollback via mev's own history) — through `mev::set_block_status`
directly, without going through a `ctx`/`SDLCState`-shaped terminal node, since `integrate_chain`'s
caller already knows exactly which `(repo, block_id)` a just-integrated step closes. An
un-injected `OrchestrationRunNode::new()` (no caller wiring `with_close_block`) still defaults to a
no-op, exactly the pre-fix behavior — nothing regresses for a caller that has not adopted this seam.
An outcome other than a clean `Closed` (an unmet operator gate, a lock held, a rejected net-new
diagnostic) is logged via `tracing::warn!` rather than silently swallowed; `close_block`'s
...
edge still returns `Err` immediately. `gates::check_permission_gate` is evaluated at the per-step
boundary alongside `check_dependencies`: under `stop_chain`, a permission denial halts the chain
and returns `Err(IntegrateError::PermissionGate)`; under `skip_dependents`, the denied step authors
the operator gate, is recorded as `Skipped` with `blocked_by: {"type": "operator", "slug": ...}`, and its
dependents are transitively skipped while independent steps proceed.
