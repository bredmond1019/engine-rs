...
| [en17k-proof-log.md](en17k-proof-log.md) | EN.17.K's Mac Mini proof log — precondition re-confirmation, the `pi`/`aider` install investigation and outcome, backend smoke dispatches, the resource probe, and the fixture-chain re-run against the Mini's serve |
...
| [trigger-and-monitor.md](trigger-and-monitor.md) | Operating runbook — fire an unattended `ORCHESTRATION`/`CONDUCTOR` run and check on it yourself over HTTP, with no live Claude Code session driving it turn by turn. Rationale split into [trigger-and-monitor-rationale.md](trigger-and-monitor-rationale.md) |
| [orchestration-test-ladder.md](orchestration-test-ladder.md) | Six incremental tiers for proving `ORCHESTRATION`/`CONDUCTOR` mechanics — gates, budget halt vs. abort, bails, coordination under concurrency, autonomous selection — each with a trigger, a success signal, and troubleshooting, without paying to test Claude Code's code quality |
