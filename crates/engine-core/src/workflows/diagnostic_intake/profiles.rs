...
/// Read `planning/harness.json`'s `diagnostic_intake.policy` section (a
/// [`PartialDiagnosticIntakePolicy`]) out of a worktree, if the file and
...
/// `diagnostic_intake.profiles[name]` entry over the built-in
