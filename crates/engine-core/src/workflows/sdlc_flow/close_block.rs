...
/// The result of one `git` invocation, as reported by a [`GitRunner`].
/// `stdout`/`stderr` are lossily decoded and trimmed — good enough for the
/// two things this module reads from them: `rev-parse --show-toplevel`'s
/// single path line, and a human-readable failure message.
#[derive(Debug, Clone)]
...
/// A single `git` invocation the manifest committer can make, injected so
/// tests can assert on the EXACT argv constructed — never `-A`, never
/// `.`, never a pathspec-less `commit` — without a real repository, while
/// the production path ([`ProcessGitRunner`]) shells out for real. Every
/// call site in [`commit_write_manifest_with_runner`] passes an explicit
/// pathspec; this trait exists so that claim is testable on the
/// constructed argv, not merely true in prose.
pub trait GitRunner {
    fn run(&mut self, cwd: &Path, args: &[&str]) -> Result<GitOutput, String>;
}

/// Production [`GitRunner`]: shells out to the real `git` binary.
...
/// The commit message every [`commit_write_manifest_with_runner`] commit
/// carries. Not policy-varying (standing rule 6 does not apply — this is a
/// fixed bookkeeping message, not a knob a run might reasonably want to
/// change for cost/speed/quality reasons).
const MANIFEST_COMMIT_MESSAGE: &str =
    "chore: commit derived fleet state regenerated by CloseBlockNode\n\n\
     Staged and committed automatically after a validated block close — see \
     crates/engine-core/src/workflows/sdlc_flow/close_block.rs.";

/// Outcome of staging + committing a [`write_manifest`] — never fatal to
/// the close itself (task 3 surfaces this on [`CloseOutcome::Closed`]; a
/// staging/commit failure is reported, not an error).
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct ManifestCommitResult {
    /// Manifest paths that were staged and landed in a commit, in the
    /// order they were processed.
    pub committed: Vec<PathBuf>,
    /// Manifest paths that could not be staged or committed, each paired
    /// with a human-readable reason.
    pub failed: Vec<(PathBuf, String)>,
}

/// Stage and commit every path in a [`write_manifest`], ONE PATH AT A
/// TIME, with an explicit pathspec on both `git add` and `git commit` —
/// never `-A`, never `.`, never a pathspec-less commit
/// (`EN.ticket.close-block-node-leaves-derived-output-uncommitted`, task
/// 2; the measured `MV.17.A` failure this exists to close).
///
/// A manifest path crosses a repo's `planning/` symlink face, so each
/// path is resolved with [`std::fs::canonicalize`] before it is staged —
/// staging the symlink face itself, rather than its resolved target,
/// would stage nothing `git` recognizes as changed. Paths are grouped by
/// the git repository each canonicalizes into (derived files can live in
/// a DIFFERENT repo than the one driving the close — see the module doc
/// comment), and each group is staged then committed independently, so a
/// path that fails to canonicalize or resolve a repo root does not take
/// down an otherwise-healthy group. Within a group, staging is
/// deliberately sequential and per-path: a single bad path must fail only
/// itself, never silently drop the whole batch (the ticket's `MV.17.A`
/// failure was exactly a batched `git add` losing files this way).
///
/// An empty manifest performs no `git` invocation at all and returns the
/// default (empty) result.
#[must_use]
pub fn commit_write_manifest(manifest: &[PathBuf]) -> ManifestCommitResult {
    let mut runner = ProcessGitRunner;
    commit_write_manifest_with_runner(manifest, &mut runner)
}

/// [`commit_write_manifest`] with an injectable [`GitRunner`] — the seam
/// tests use to assert on constructed argv and to drive a real temporary
/// git repository without touching this crate's own working tree.
pub fn commit_write_manifest_with_runner(
    manifest: &[PathBuf],
    runner: &mut dyn GitRunner,
) -> ManifestCommitResult {
    let mut result = ManifestCommitResult::default();
    if manifest.is_empty() {
        return result;
    }

    // Group canonicalized manifest paths by the git repository each
    // resolves into: `(repo_root, [(original_path, path_relative_to_repo_root)])`.
    let mut groups: Vec<(PathBuf, Vec<(PathBuf, String)>)> = Vec::new();
    ...
    for path in manifest {
        let canonical = match std::fs::canonicalize(path) {
            ...
            Err(e) => {
                result
                    .failed
                    .push((path.clone(), format!("canonicalize failed: {e}")));
                continue;
            }
        };
        let parent = canonical.parent().unwrap_or(canonical.as_path());
        let toplevel = match runner.run(parent, &["rev-parse", "--show-toplevel"]) {
            Ok(out) if out.success => out.stdout,
            Ok(out) => {
                result.failed.push((
                    path.clone(),
                    format!("git rev-parse --show-toplevel failed: {}", out.stderr),
                ));
                continue;
            }
            Err(e) => {
        ...
        let repo_root = match std::fs::canonicalize(PathBuf::from(toplevel.trim())) {
            Ok(r) => r,
            Err(e) => {
                result.failed.push((
                    path.clone(),
                    format!("could not canonicalize repo root '{toplevel}': {e}"),
                ));
                continue;
            }
        };
        let relative = match canonical.strip_prefix(&repo_root) {
            Ok(rel) => rel.to_string_lossy().to_string(),
            Err(_) => {
                result.failed.push((
                    path.clone(),
                    format!(
                        "canonicalized path {} is not under its own repo toplevel {}",
                        canonical.display(),
                        repo_root.display()
                    ),
                ));
        ...
        match groups.iter_mut().find(|(root, _)| *root == repo_root) {
            Some((_, entries)) => entries.push((path.clone(), relative)),
            None => groups.push((repo_root, vec![(path.clone(), relative)])),
        }
    }

    for (repo_root, entries) in groups {
        let mut staged: Vec<(PathBuf, String)> = Vec::new();
        for (original, relative) in entries {
            // ONE PATH AT A TIME, explicit pathspec — never `-A`/`.`.
            match runner.run(&repo_root, &["add", "--", relative.as_str()]) {
                Ok(out) if out.success => staged.push((original, relative)),
                Ok(out) => result
                    .failed
                    .push((original, format!("git add failed: {}", out.stderr))),
                Err(e) => result.failed.push((original, e)),
            }
        }
        if staged.is_empty() {
            continue;
        ...
        // Explicit pathspec on the commit too (`-o` + `--` + the exact
        // relative paths just staged) — never a pathspec-less commit. One
        // HQ git repo tracks every repo's `planning/`, so a bare commit
        // would sweep in a concurrent lane's unrelated staged work.
        let mut args: Vec<&str> = vec!["commit", "-o", "-m", MANIFEST_COMMIT_MESSAGE, "--"];
        for (_, relative) in &staged {
            args.push(relative.as_str());
        }
        match runner.run(&repo_root, &args) {
            Ok(out) if out.success => {
                result.committed.extend(staged.into_iter().map(|(p, _)| p));
            }
            Ok(out) => {
                let reason = format!("git commit failed: {}", out.stderr);
                result
                    .failed
                    .extend(staged.into_iter().map(|(p, _)| (p, reason.clone())));
            }
            Err(e) => {
                result
                    .failed
                    .extend(staged.into_iter().map(|(p, _)| (p, e.clone())));
            }
        }
    }
...
fn attempt_close_with_validator(
    ...
    // affect the `wrote` boolean or the rollback decision below.
    let write_manifest_paths = write_manifest(&close_report);
    ...
    if net_new.is_empty() {
        // The close validated clean — stage and commit the derived fallout
        // now, never on a path that is about to roll back (see the arm
        // below: a `Rejected` close commits nothing, by construction,
...
mod tests {
    ...
    // --- commit_write_manifest_with_runner (task 2) -----------------------

    /// Records every `(cwd, args)` a [`GitRunner`] call receives, and
    /// answers with a caller-programmed script keyed on the joined argv —
    /// so a unit test can assert on the EXACT argv constructed (no `-A`,
    /// no `.`, always an explicit pathspec) without touching a real `git`
    /// binary or filesystem.
    #[derive(Default)]
    struct FakeGitRunner {
        calls: Vec<(PathBuf, Vec<String>)>,
        /// argv (space-joined) -> canned response. Anything unmatched
        /// succeeds with empty output — good enough for `add`/`commit`,
        /// which this suite never needs stdout from.
        scripted: HashMap<String, GitOutput>,
    }

    impl FakeGitRunner {
        fn script(&mut self, argv: &[&str], output: GitOutput) {
            self.scripted.insert(argv.join(" "), output);
        }
    }
    ...
    impl GitRunner for FakeGitRunner {
        fn run(&mut self, cwd: &Path, args: &[&str]) -> Result<GitOutput, String> {
            ...
            let key = args.join(" ");
            Ok(self.scripted.get(&key).cloned().unwrap_or(GitOutput {
                success: true,
                stdout: String::new(),
                stderr: String::new(),
            }))
    ...
    #[test]
    fn empty_manifest_makes_no_git_call_at_all() {
        let mut runner = FakeGitRunner::default();
        let result = commit_write_manifest_with_runner(&[], &mut runner);
        assert_eq!(result, ManifestCommitResult::default());
        assert!(
            runner.calls.is_empty(),
            "an empty manifest must invoke git zero times"
        );
    }

    /// A real temporary git repo (not a fake) is what task 2's other
    /// acceptance criteria (symlinked `planning/` face, decoy file,
    /// per-path failure reporting) drive against — see
    /// `crates/engine-core/tests/it/close_block_commit_manifest.rs`. This
    /// unit test's job is narrower and argv-exact: prove the constructed
    /// invocations never carry `-A`/`.` and the commit always carries an
    /// explicit pathspec after `--`, on a manifest of two files with a
    /// scripted repo root.
    #[test]
    fn never_constructs_a_batched_or_pathspec_less_git_invocation() {
        let dir = std::env::temp_dir().join(format!(
            "close-block-fake-git-{}-{}",
            std::process::id(),
        ...
        std::fs::write(dir.join("planning/a.md"), "a").unwrap();
        std::fs::write(dir.join("planning/b.md"), "b").unwrap();
        ...
        runner.script(
            ...
            GitOutput {
                success: true,
                stdout: dir.to_string_lossy().to_string(),
                stderr: String::new(),
            },
        );

        let manifest = vec![dir.join("planning/a.md"), dir.join("planning/b.md")];
        ...
        for (_, args) in &runner.calls {
            assert_ne!(
                args.as_slice(),
                ["add", "-A"],
                "must never batch-stage with -A"
            );
            assert_ne!(
                args.as_slice(),
                ["add", "."],
                "must never batch-stage with ."
            );
            if args.first().map(String::as_str) == Some("add") {
                assert_eq!(
                    args.len(),
                    3,
                    "git add must be `add -- <one path>`: {args:?}"
                );
                assert_eq!(args[1], "--");
            }
            if args.first().map(String::as_str) == Some("commit") {
                assert!(
                    args.iter().any(|a| a == "--"),
                    "commit must carry an explicit `--` pathspec separator: {args:?}"
                );
                let dashdash = args.iter().position(|a| a == "--").unwrap();
                assert!(
                    dashdash + 1 < args.len(),
                    "commit's pathspec after `--` must not be empty: {args:?}"
                );
            }
        }
    ...
    #[test]
    fn a_path_that_fails_to_canonicalize_is_reported_not_fatal_to_the_rest() {
        let dir = std::env::temp_dir().join(format!(
            "close-block-fake-git-missing-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let manifest = vec![dir.join("does-not-exist.md"), dir.join("real.md")];
        let result = commit_write_manifest_with_runner(&manifest, &mut runner);

        assert_eq!(result.committed, vec![dir.join("real.md")]);
        assert_eq!(result.failed.len(), 1);
        assert_eq!(result.failed[0].0, dir.join("does-not-exist.md"));
  ...
  "tracks": [{{ "title": "P1", "blocks": [{}] }}] }}"#,
    ...
    fn net_new_diagnostic_rolls_back_and_leaves_block_open() {
        ...
        // A real git repo over the fixture — a `Rejected` close (task 2's
        // acceptance criteria) must commit NOTHING. `commit_write_manifest`
        // is only ever called on the `net_new.is_empty()` branch of
        // `attempt_close_with_validator`, so a `Rejected` outcome proves
        // this by construction; this assertion turns that into a runtime
        // check rather than leaving it as a claim about the source.
        std::process::Command::new("git")
            .current_dir(&root)
            .args(["init", "-q"])
            .status()
            .expect("git init");
        let no_head = std::process::Command::new("git")
            .current_dir(&root)
            .args(["rev-parse", "HEAD"])
            .output()
            .expect("git rev-parse");
        assert!(
            !no_head.status.success(),
            "fresh repo must start with no commits"
        );
        ...
        // Task 2 AC: a `Rejected` close commits nothing — still no `HEAD`.
        let still_no_head = std::process::Command::new("git")
            .current_dir(&root)
            .args(["rev-parse", "HEAD"])
            .output()
            .expect("git rev-parse");
        assert!(
            !still_no_head.status.success(),
            "a Rejected close must never commit anything"
        );
    }
